...
function Template6() {
    ...
    return (
        <div style={{ backgroundColor: "#ffffff", padding: "20px", fontFamily: "Arial, sans-serif" }}>
            <div style={{ textAlign: "center" }}>
                <h1 style={{ margin: "0", fontSize: "2.5em", color: "#333" }}>
                    {personalInfo.firstName} {personalInfo.lastName}
                </h1>
                <h3 style={{ margin: "5px 0", color: "#777" }}>{personalInfo.jobTitle}</h3>
                <p style={{ margin: "5px 0", color: "#555" }}>{personalInfo.Email} | {personalInfo.Mobile}</p>
                <p style={{ margin: "5px 0", color: "#555" }}>
                    {personalInfo.Address1}, {personalInfo.Address2}, {personalInfo.City}, {personalInfo.State}, {personalInfo.Pin}
                </p>
            ...
            <div style={{ margin: "20px 0" }}>
                ...
                <p style={{ fontSize: "1.1em", color: "#555" }}>{personalInfo.Objective}</p>
            </div>
            ...
            <div style={{ margin: "20px 0" }}>
                ...
                {dataStore.workEx.map((item) => (
                    <div key={shortid.generate()} style={{ marginBottom: "15px" }}>
                        <h4 style={{ margin: "0", color: "#333" }}>{item.title} at {item.orgName}</h4>
                        <p style={{ margin: "5px 0", color: "#777" }}>
                            {item.startYear} - {item.endYear}
                        </p>
                        <p style={{ fontSize: "1em", color: "#555" }}>{item.jobDescription}</p>
                    </div>
                ))}
            </div>
